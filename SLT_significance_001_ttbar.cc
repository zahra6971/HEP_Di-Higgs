...
void SLT_significance_001_ttbar(){
  ...
  string h_titles[n_smp] = { "ttbar", "VBF001 x 67936.96" };  // only for legend 67936.96
  ...
  //   ===========================================================   pTBB   ===========================================================
    ...
    for(int i = 0; i<n_smp; i++){
      ...
      else if(i == 1){chain_smp[i]->Draw(hist_draw[i].c_str(), "67936.96*(weight)*((BTag==2)&&((LepQ)*(TauQ)<0))", "SAMEHIST");} //VBF001

    }
    
    for(int i = 0; i<n_smp; i++){
        if(i == 1){chain_smp[i]->Draw(hist_draw[i].c_str(), "67936.96*(weight)*((BTag==2)&&((LepQ)*(TauQ)<0))", "SAMEHIST");} //VBF001
    }
   ...
   TH1F *h1_2 = new TH1F("h1_2","Quadratic Sum from right",60,0,1200); //right
    ...
    TGaxis*axis32 = new TGaxis(3.5,0,3.5,3000,0,rightmax32,510,"+L");
      ...
      c32 -> SaveAs("dPhiLep0MET_001.pdf");
    ...
    TGaxis*axis33 = new TGaxis(3.5,0,3.5,3500,0,rightmax33,510,"+L");
      ...
      c33 -> SaveAs("dPhiLep1MET_001.pdf");
    ...
    h_hist[1]->SetLineColor(kRed); h_hist[1]->SetMarkerColor(kRed); //VBF_SM
      ...
      for(int i = 0; i<n_smp; i++){
        ...
        if (i == 0){chain_smp[i]->Draw(hist_draw[i].c_str(), "(weight)*((BTag==2)&&((LepQ)*(TauQ)<0)&&(MT2>260000)&&(sqrt( ((Mw*0.001-80.4)/(Mw*0.001)) * ((Mw*0.001-80.4)/(Mw*0.001)) + ((Mt*0.001-172.5)/(Mt*0.001)) * ((Mt*0.001-172.5)/(Mt*0.001)) )>0.3))", "SAMEHIST");} //ttbar
        else if(i == 1){chain_smp[i]->Draw(hist_draw[i].c_str(), "(weight)*((BTag==2)&&((LepQ)*(TauQ)<0)&&(MT2>260000)&&(sqrt( ((Mw*0.001-80.4)/(Mw*0.001)) * ((Mw*0.001-80.4)/(Mw*0.001)) + ((Mt*0.001-172.5)/(Mt*0.001)) * ((Mt*0.001-172.5)/(Mt*0.001)) )>0.3))", "SAMEHIST");} //VBF001
       
      }
    /*
    for(int i = 0; i<n_smp; i++){
        if(i == 1){chain_smp[i]->Draw(hist_draw[i].c_str(), "(weight)*((BTag==2)&&((LepQ)*(TauQ)<0))", "SAMEHIST");} //VBF001
    }*/
    ...
    myfile <<"                                                                                    "<<endl;
     ...
     entries_VBF001 = h_hist[1]->GetEntries();
     yields_VBF001 = h_hist[1]->Integral() / 67936.96; // multiply this factor on VBF001 (plot)
     myfile<<"\t" <<h_title[1] <<"\t \t"<<entries_VBF001<<"\t \t"<<yields_VBF001<<endl;
     myfile <<"                                                                                   "<<endl;
