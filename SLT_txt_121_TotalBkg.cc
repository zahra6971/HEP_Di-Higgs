...
void SLT_txt_121_TotalBkg(){
  ...
  string h_titles[n_smp] = { "SM H", "diboson", "DY", "Fake", "Z", "Wl#nu", "Z#tau#tau", "single top", "ttbar", "VBF121 x 248634.031", };  // only for legend
  ...
  string hist_draw[n_smp];*/
    ...
    const int n_smp = 11;
    string smp_tree[n_smp] = { "SMH", "diboson", "DY", "ttbar", "W", "Z", "W", "Ztt", "singletop", "ttbar", "VBF121" };
    string smp;
    TChain  *chain_smp[n_smp];
    ...
    string ana_tag = "FinalNtuple.TauLH.";
    string smp_names[n_smp] = { "SMH", "diboson", "DY", "ttbar", "W", "Zll", "W", "Ztautau", "singletop", "ttbar", "VBF.hh_bbtt_vbf_lh_l1cvv2cv1." };
    ...
    string h_titles[n_smp] = { "SM H", "diboson", "DY", "Fake_ttbar", "Fake_W", "Zee/#mu#mu", "W", "Z#tau#tau", "single top", "ttbar", "VBF121 x 248633.791", };  // only for legend
      
    string h_title[n_smp] = { "SMH", "diboson", "DY", "Fake_ttbar", "Fake_W", "Z", "W", "Ztt", "singletop", "ttbar", "VBF121", };
  ...
  h_hist[10]->SetLineColor(kRed); h_hist[10]->SetMarkerColor(kRed); //VBF_SM
    ...
    for(int i = 0; i<n_smp; i++){
      ...
      else if(i == 10){chain_smp[i]->Draw(hist_draw[i].c_str(), "(weight)*((BTag==2)&&((LepQ)*(TauQ)<0))", "SAMEHIST");} //VBF121
  ...
  ofstream myfile;
  myfile.open ("Yields_121_TotalBkg.txt");
  myfile <<"---------------------------------------------------"<<endl;
  myfile <<"                                                                                    "<<endl;
  ...
  for(int i= 10; i>=0 ; i--)
  {
   entries_VBF121 = h_hist[10]->GetEntries();
      yields_VBF121 = h_hist[10]->Integral();
  }
    myfile<<"\t" <<h_title[10] <<"\t \t"<<entries_VBF121<<"\t \t"<<yields_VBF121<<endl;
    myfile <<"                                                                                   "<<endl;
   ...
   sum_yields_noVBF121 += yields;
   sum_entries_noVBF121 += entries;
  }
    
  myfile<<"\t" << "Bkg :" <<"\t \t"<< sum_entries_noVBF121 <<"\t \t"<<sum_yields_noVBF121<<endl;
  myfile <<"                                                                                   "<<endl;
  myfile <<"---------------------------------------------------"<<endl;
  myfile.close();
