...
void SLT_txt_1011_TotalBkg(){
  ...
  string h_titles[n_smp] = { "SM H", "diboson", "DY", "Fake", "Z", "Wl#nu", "Z#tau#tau", "single top", "ttbar", "VBF1011 x 248634.031", };  // only for legend
  ...
  string hist_draw[n_smp];*/
    ...
    const int n_smp = 11;
    string smp_tree[n_smp] = { "SMH", "diboson", "DY", "ttbar", "W", "Z", "W", "Ztt", "singletop", "ttbar", "VBF1011" };
    string smp;
    TChain  *chain_smp[n_smp];
    ...
    string ana_tag = "FinalNtuple.TauLH.";
    string smp_names[n_smp] = { "SMH", "diboson", "DY", "ttbar", "W", "Zll", "W", "Ztautau", "singletop", "ttbar", "VBF.hh_bbtt_vbf_lh_l10cvv1cv1." };
    ...
    string h_titles[n_smp] = { "SM H", "diboson", "DY", "Fake_ttbar", "Fake_W", "Zee/#mu#mu", "W", "Z#tau#tau", "single top", "ttbar", "VBF1011 x 248633.791", };  // only for legend
      
    string h_title[n_smp] = { "SMH", "diboson", "DY", "Fake_ttbar", "Fake_W", "Z", "W", "Ztt", "singletop", "ttbar", "VBF1011", };
  ...
  h_hist[10]->SetLineColor(kRed); h_hist[10]->SetMarkerColor(kRed); //VBF_SM
    ...
    for(int i = 0; i<n_smp; i++){
      ...
      if (i == 3 ){chain_smp[i]->Draw(hist_draw[i].c_str(),"(weight)*((BTag==2)&&((LepQ)*(TauQ)<0)&&(isFakeTau==1)&&(sqrt( ((mMMC*0.001-118)/(mMMC*0.001)) * ((mMMC*0.001-118)/(mMMC*0.001)) + ((mBB*0.001-120)/(mBB*0.001)) * ((mBB*0.001-120)/(mBB*0.001)) )<0.3))",  "SAMEHIST");} //Fake
      else if (i == 4 ){chain_smp[i]->Draw(hist_draw[i].c_str(),"(weight)*((BTag==2)&&((LepQ)*(TauQ)<0)&&(isFakeTau==1)&&(sqrt( ((mMMC*0.001-118)/(mMMC*0.001)) * ((mMMC*0.001-118)/(mMMC*0.001)) + ((mBB*0.001-120)/(mBB*0.001)) * ((mBB*0.001-120)/(mBB*0.001)) )<0.3))",  "SAMEHIST");} //Fake_W
      else if (i == 6 ){chain_smp[i]->Draw(hist_draw[i].c_str(),"(weight)*((BTag==2)&&((LepQ)*(TauQ)<0)&&(isFakeTau==0)&&(sqrt( ((mMMC*0.001-118)/(mMMC*0.001)) * ((mMMC*0.001-118)/(mMMC*0.001)) + ((mBB*0.001-120)/(mBB*0.001)) * ((mBB*0.001-120)/(mBB*0.001)) )<0.3))",  "SAMEHIST");} //Wlv
      else if(i == 9){chain_smp[i]->Draw(hist_draw[i].c_str(), "(weight)*((BTag==2)&&((LepQ)*(TauQ)<0)&&(isFakeTau==0)&&(sqrt( ((mMMC*0.001-118)/(mMMC*0.001)) * ((mMMC*0.001-118)/(mMMC*0.001)) + ((mBB*0.001-120)/(mBB*0.001)) * ((mBB*0.001-120)/(mBB*0.001)) )<0.3))", "SAMEHIST");} //ttbar
      else if(i == 10){chain_smp[i]->Draw(hist_draw[i].c_str(), "(weight)*((BTag==2)&&((LepQ)*(TauQ)<0)&&(sqrt( ((mMMC*0.001-118)/(mMMC*0.001)) * ((mMMC*0.001-118)/(mMMC*0.001)) + ((mBB*0.001-120)/(mBB*0.001)) * ((mBB*0.001-120)/(mBB*0.001)) )<0.3))", "SAMEHIST");} //VBF1011
      else chain_smp[i]->Draw(hist_draw[i].c_str(),"(weight)*((BTag==2)&&((LepQ)*(TauQ)<0)&&(sqrt( ((mMMC*0.001-118)/(mMMC*0.001)) * ((mMMC*0.001-118)/(mMMC*0.001)) + ((mBB*0.001-120)/(mBB*0.001)) * ((mBB*0.001-120)/(mBB*0.001)) )<0.3))",  "SAMEHIST");
  ...
  int entries_VBF1011;
  float yields;
  float yields_VBF1011;
  int sum_entries;
  int sum_entries_noVBF1011;
  float sum_yields_noVBF1011;
  float sum_yields;
    
  ofstream myfile;
  myfile.open ("Yields_1011_TotalBkg.txt");
  myfile <<"---------------------------------------------------"<<endl;
  myfile <<"                                                                                    "<<endl;
  ...
  for(int i= 10; i>=0 ; i--)
  {
   entries_VBF1011 = h_hist[10]->GetEntries();
      yields_VBF1011 = h_hist[10]->Integral();
  }
    myfile<<"\t" <<h_title[10] <<"\t \t"<<entries_VBF1011<<"\t \t"<<yields_VBF1011<<endl;
    myfile <<"                                                                                   "<<endl;
   ...
   sum_yields_noVBF1011 += yields;
   sum_entries_noVBF1011 += entries;
  }
    
  myfile<<"\t" << "Bkg :" <<"\t \t"<< sum_entries_noVBF1011 <<"\t \t"<<sum_yields_noVBF1011<<endl;
  myfile <<"                                                                                   "<<endl;
  myfile <<"---------------------------------------------------"<<endl;
  myfile.close();
